class HiveTableConstant {
  HiveTableConstant._();

  //Database name

  static const String dbName = "hotelspot_db";

  static const int hotelId = 0;
  static const String hotelTable = 'hotel_table';

  static const int authTypeId = 1;
  static const String authTable = 'auth_table';

  static const int bookingId = 2;
  static const String bookingTable = 'booking_table';

  static const int favouriteId = 3;
  static const String favouriteTable = 'favourite_table';

  static const int reviewId = 4;
  static const String reviewTable = 'review_table';
}
